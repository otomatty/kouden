...
alter table "public"."offerings" add constraint "offerings_kouden_entry_id_fkey" FOREIGN KEY (kouden_entry_id) REFERENCES kouden_entries(id) ON DELETE SET NULL not valid;

alter table "public"."offerings" validate constraint "offerings_kouden_entry_id_fkey";

alter table "public"."offerings" add constraint "offerings_type_check" CHECK ((type = ANY (ARRAY['FLOWER'::text, 'INCENSE'::text, 'FOOD'::text, 'MONEY'::text, 'OTHER'::text]))) not valid;
...
create policy "ユーザーは自分が作成した関係性を削除できる"
on "public"."relationships"
as permissive
for delete
to public
using (((created_by = auth.uid()) AND (NOT is_default)));


create policy "ユーザーは自分が作成した関係性を更新できる"
on "public"."relationships"
as permissive
for update
to public
...
create policy "ユーザーは自分が作成した香典帳に関係性を追"
on "public"."relationships"
as permissive
for insert
to public
...
create policy "ユーザーは自分が作成した香典帳の関係性を閲"
on "public"."relationships"
as permissive
for select
to public
...
for select
to authenticated
using ((((status)::text = 'published'::text) AND ((published_at IS NULL) OR (published_at <= now())) AND ((expires_at IS NULL) OR (expires_at > now()))));
...
CREATE TRIGGER trigger_add_kouden_owner AFTER INSERT ON public.koudens FOR EACH ROW EXECUTE FUNCTION add_kouden_owner();
