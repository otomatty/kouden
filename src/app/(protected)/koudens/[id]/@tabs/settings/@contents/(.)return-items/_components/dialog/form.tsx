...
// フォームの型定義
type ReturnItemFormValues = z.infer<typeof formSchema>;
...
export function ReturnItemForm({
	...
	const onSubmit = async (data: ReturnItemFormValues) => {
		try {
			if (defaultValues?.name) {
				// 更新
				await updateReturnItem({
					id: (defaultValues as ReturnItem).id,
					kouden_id: koudenId,
					...data,
				});
				toast({
...
// 型定義のエクスポート
export type { ReturnItemFormProps, ReturnItemFormValues };
